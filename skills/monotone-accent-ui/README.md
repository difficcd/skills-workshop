...
Claude Code / Claude Agent SDK가 실행할 수 있는 **skill** 로 정리한 것.

> A Claude Code skill for building monotone UI carried by a single accent color.
> Design tokens, an accent-intensity ladder, 14 component recipes, and a zero-dependency violation checker.
> Everything is a number or a runnable command — no "make it clean" advice.
...
원하는 라이선스를 골라 `LICENSE` 파일을 추가하세요 (MIT 권장).
