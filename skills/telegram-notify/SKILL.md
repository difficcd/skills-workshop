...
from the project directory (or set `TG_SESSION_DIR`); an unregistered directory - a scratch folder, a
temp worktree - is **refused** (exit 3) rather than sent anonymous. `TG_UNTAGGED=1` sends anyway, on purpose.
