...
| # | 규칙 | 이유 |
|---|---|---|
| 0-1 | **자동·주기 전송을 만들지 않는다.** heartbeat/poll 스크립트, cron, 스케줄러, 전송하는 hook, 반복 감시 태스크 — 전부 금지 | N분마다 오는 "지금 X 하는 중"은 정보가 아니라 소음이다. 한 번 만들면 사용자가 직접 꺼야 한다 |
...
```
🟢 easy-mv-maker · 18:27          ← 막히면 🔴, 알림 미리보기만으로 구분된다
지금: 게이트 통과 후 dev 트리 머지
직전: #150 커서 8방향
다음: #137 spine 머지
⎇ check/all · ● Merge origin/fix/… (12분 전)   ← 기계가 읽는다. 에이전트가 못 쓴다
```
