...
```bash
node --test skills/telegram-notify/test/route.test.mjs    # 테스트 19개
```
...
첫 번째를 끊는다 — 실측: `Conflict: terminated by other getUpdates request` — 그러면 끊긴 쪽이
기록하고 5초 기다렸다 다시 물어서, 둘이 번갈아 잡는다. 잃는 것은 없다: 이긴 요청이 둘 몫을 다
스풀에 넣고, 진 쪽은 다음 회차에 자기 몫을 찾는다. 비용은 진 쪽의 몇 초 지연과 stderr 한 줄이다.
감시자는 사용자가 "여기로 보내겠다"고 한 세션에 켜고, 없는 세션도 다음 Stop 에서 메시지를 받는다.
