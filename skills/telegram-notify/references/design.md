# Telegram notify — the long form

The design of this skill, with the incidents behind each decision. `SKILL.md` is the short
version the agent reads on every load; this is where the reasoning lives. Open it when a rule in
`SKILL.md` seems arbitrary, when changing the mechanism, or when something behaves in a way the
short version does not explain.
...
Field rules and how to word them: **[reporting.md](reporting.md)**.
...
switch, are in [`reachability.md`](reachability.md).
...
must be done when building one: **[reachability.md](reachability.md)**.
...
- exit `2` → needs setting up. Open **[setup.md](setup.md)** and follow it
...
Failure codes and their causes are in [setup.md](setup.md).
