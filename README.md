...
| [**ask-first**](skills/ask-first/) ([Korean](skills/ask-first/README.ko.md)) | **When a request can be read more than one way, the agent asks before it builds** — at a threshold you set: `high` only on a real fork with an expensive guess, `mid` on any material fork, `low` at the slightest doubt. One question a turn, recommended reading first, a default so an absent user is not a blocked one. A hook keeps the level across sessions |
