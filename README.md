...
| [**telegram-notify**](skills/telegram-notify/) | **컴퓨터만 켜 두면 폰의 텔레그램으로 세션들을 관리하고 지시한다.** 에이전트는 보고하고, 사용자는 답장으로 지시하고, 번호로 세션을 고르고(`1 …`, `1,3 …`, `* …`), 놀고 있는 세션은 메시지가 오면 깨어난다. 2분 연동, 보낼 때/안 보낼 때의 기준, **절반을 기계가 채워 "세션이 아직 살아 있나"에 답하는 고정 보고 형식**, 실측한 오버헤드. Node 18+, 의존성 0 |
